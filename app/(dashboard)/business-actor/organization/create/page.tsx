...
export default function CreateOrganizationPage() {
  ...
  const { setActiveOrganization, fetchUserOrganizationsList } = useActiveOrganization();
  ...
  const handleCreateSuccess = async (newOrgData: OrganizationDto) => {
    ...
    if (newOrgData.organization_id) {
      ...
      if (session?.user?.businessActorId) {
        ...
        toast.error("Could not fetch organizations list: missing user session.");
      }
      ...
      toast.error("Could not activate the new organization. Redirecting to dashboard.");
      router.push(`/business-actor/dashboard`);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto">
       <div className="mb-8 text-center">
        ...
        <p className="text-muted-foreground mt-2">This is the first step to unlocking your business workspace. Fill out the details below.</p>
      </div>
      <OrganizationForm onSuccessAction={handleCreateSuccess} />
    </div>
  );
}
