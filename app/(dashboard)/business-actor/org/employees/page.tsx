...
export default function ManageEmployeesPage() {
  ...
  const executeDelete = async () => {
    ...
    setAllItems(prev => prev.filter(item => !idsToDelete.includes(item.employee_id!)));
    setIsDeleteDialogOpen(false);

    const promise = Promise.all(
      itemsToDelete.map((item) =>
        organizationRepository.deleteOrgEmployee(activeOrganizationId, item.employee_id!)
      )
    );
  ...
  const departmentOptions: DataTableFilterOption[] = useMemo(() => {
    const departments = new Set(allItems.map((item) => item.department).filter(Boolean));
  ...
  const columns = useMemo<ColumnDef<EmployeeDto>[]>(
    () => getEmployeeColumns({ onEditAction: handleEditAction, onDeleteAction: (item) => handleDeleteConfirmation([item]) }, agencies),
    [agencies]
  );
      ...
      <ResourceDataTable
        ...
        pageHeader={
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Employee Roster</h1>
              <p className="text-muted-foreground">
                Manage team members for <b>{activeOrganizationDetails?.long_name}</b>.
              </p>
            </div>
            <Button onClick={() => router.push("/business-actor/org/employees/create")} size="sm" className="h-10">
        ...
        filterControls={(table) => (
          <>
            {table.getColumn("employee_role") && <DataTableFacetedFilter column={table.getColumn("employee_role")} title="Role" options={roleOptions} />}
            {table.getColumn("department") && <DataTableFacetedFilter column={table.getColumn("department")} title="Department" options={departmentOptions} />}
            {table.getColumn("agency_id") && <DataTableFacetedFilter column={table.getColumn("agency_id")} title="Assignment" options={agencyOptions} />}
          </>
        )}
        ...
        emptyState={
          <FeedbackCard
            ...
            actionButton={
              <Button onClick={() => router.push("/business-actor/org/employees/create")}>
                <PlusCircle className="mr-2 h-4 w-4" /> Add Employee
              </Button>
            }
          />
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete <strong>{itemsToDelete.length} employee(s)</strong>.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
