...
} from "@/types/organization"; // Added GetThirdPartyRequest
...
import {
  ThirdPartyForm,
  ThirdPartyFormData,
} from "@/components/organization/third-parties/third-party-form";
...
const typeOptions: DataTableFilterOption[] = ThirdPartyTypeValues.map((t) => ({
  value: t,
  label: t,
}));
...
export function OrgThirdPartiesClientPage() {
  ...
  const [editingItem, setEditingItem] = useState<ThirdPartyDto | undefined>(
    undefined
  );
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      ...
      const params: GetThirdPartyRequest = { page: 1, size: 100, type:  '11'};
      const data = await organizationRepository.getThirdParties(
  ...
  const executeDelete = async () => {
    ...
    const promise = Promise.all(
      itemsToDelete.map((item) =>
        organizationRepository.deleteThirdParty(activeOrganizationId, item.id!)
      )
    );
  ...
  const handleFormSubmit = async (
    ...
    try {
      const promise = editingItem?.id
        ? organizationRepository.updateThirdParty(
            activeOrganizationId,
            editingItem.id,
            data
          )
        : organizationRepository.createThirdParty(
            activeOrganizationId,
            data.type,
            data
          );
  ...
  const columns = useMemo<ColumnDef<ThirdPartyDto>[]>(
    () =>
      getThirdPartyColumns({
  ...
  if (!activeOrganizationId && !isLoading) {
    return (
      <FeedbackCard
        ...
        description="Please select an active organization to manage third-parties."
      />
    );
  }
    ...
    <>
      <ResourceDataTable
        ...
        pageHeader={
          <PageHeader
            title="Third-Parties"
            description={`Manage external partners for ${activeOrganizationDetails?.long_name}`}
            action={
              <Button onClick={() => handleOpenFormModal()}>
                <PlusCircle className="mr-2 h-4 w-4" /> Add Third-Party
              </Button>
            }
          />
        }
        ...
        emptyState={
          <FeedbackCard
            icon={Briefcase}
            title="No Third-Parties Yet"
            description="Add your first external partner, supplier, or other entity."
            actionButton={
              <Button onClick={() => handleOpenFormModal()}>
                <PlusCircle className="mr-2 h-4 w-4" /> Add Third-Party
              </Button>
        ...
        filteredEmptyState={
          <FeedbackCard
            icon={SearchIcon}
            title="No Third-Parties Found"
            description="Your search did not match any partners."
          />
        }
      />
      <Dialog open={isFormModalOpen} onOpenChange={setIsFormModalOpen}>
        <DialogContent>
          <DialogTitle className="sr-only">
            {editingItem ? "Edit Third-Party" : "Add New Third-Party"}
          </DialogTitle>
          <ThirdPartyForm
