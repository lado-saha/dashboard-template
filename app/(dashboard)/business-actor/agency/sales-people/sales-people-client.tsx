...
export function AgencySalesPeopleClientPage() {
  ...
  const refreshData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await organizationRepository.getAgencySalesPersons(
  ...
  const handleFormSubmit = async (
    ...
    try {
      const promise = editingSalesPerson?.sales_person_id
        ? organizationRepository.updateAgencySalesPerson(
            activeOrganizationId,
            activeAgencyId,
            editingSalesPerson.sales_person_id,
            data as UpdateSalesPersonRequest
          )
        : organizationRepository.createAgencySalesPerson(
            activeOrganizationId,
            activeAgencyId,
            data as CreateSalesPersonRequest
          );

      await toast.promise(promise, {
        loading: `${
          editingSalesPerson ? "Updating" : "Creating"
        } sales person...`,
        success: `Sales person ${
          editingSalesPerson ? "updated" : "created"
        } successfully!`,
  ...
  if (!activeAgencyId && !isLoading) {
    return (
      <FeedbackCard
        ...
        description="Please select an active agency to manage its sales people."
      />
    );
  }
  ...
  return (
    <>
      <ResourceDataTable
        ...
        pageHeader={
          <PageHeader
            title="Agency Sales People"
            description={`Manage the sales team for ${activeAgencyDetails?.long_name}`}
            action={
        ...
        emptyState={
          <FeedbackCard
            icon={UserCheck}
            title="No Sales People in this Agency"
            description="Add your first sales person to this agency's team."
            actionButton={
              <Button onClick={() => handleOpenFormModal()}>
                <PlusCircle className="mr-2 h-4 w-4" /> Add Sales Person
              </Button>
        ...
        filteredEmptyState={
          <FeedbackCard
            ...
            description="Your search did not match any sales people in this agency."
          />
        }
      />
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{itemsToDelete.length} sales person(s)</strong> from this
              agency.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
