...
export function AgencyCustomersClientPage() {
  ...
  const refreshData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await organizationRepository.getAgencyCustomers(
  ...
  const executeDelete = async () => {
    ...
    const promise = Promise.all(
      itemsToDelete.map((item) =>
        organizationRepository.deleteAgencyCustomer(
          activeOrganizationId,
          activeAgencyId,
          item.customer_id!
        )
  ...
  const handleFormSubmit = async (
    ...
    try {
      const promise = editingCustomer?.customer_id
        ? organizationRepository.updateAgencyCustomer(
            activeOrganizationId,
            activeAgencyId,
            editingCustomer.customer_id,
            data as UpdateCustomerRequest
          )
        : organizationRepository.createAgencyCustomer(
            activeOrganizationId,
            activeAgencyId,
  ...
  if (!activeAgencyId && !isLoading) {
    return (
      <FeedbackCard
        ...
        description="Please select an active agency to manage its customers."
      />
    );
  }
      ...
      <ResourceDataTable
        ...
        pageHeader={
          <PageHeader
            ...
            description={`Manage customers for ${
        ...
        emptyState={
          <FeedbackCard
            icon={Users}
            title="No Customers in this Agency"
            description="Add your first customer to this agency."
            actionButton={
              <Button onClick={() => handleOpenFormModal()}>
                <PlusCircle className="mr-2 h-4 w-4" /> Add Customer
              </Button>
        ...
        filteredEmptyState={
          <FeedbackCard
            ...
            description="Your search did not match any customers in this agency."
          />
        }
      />
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{itemsToDelete.length} customer(s)</strong> from this
              agency.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
